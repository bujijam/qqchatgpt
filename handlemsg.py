...
#这里的内容会在`HandleMsg`实例化时或每次重置完对话后第一个发给ChatGPT
#可以给chatgpt洗脑从而解除一些限制，这里举例洗脑成猫娘，你也可以自行改动（
...
class HandleMsg:
    def initialize(self, gid):
        ...
        all_convo[gid] = chatbot.get_conversations()[0]['id']
        print()
    ...
    def gro_msg(self, gid, msg, nick):
        ...
        self.message = ''
        try:
            for data in chatbot.ask(msg, conversation_id = all_convo[gid]):
                self.message = data["message"]
        except KeyError:
            self.initialize(gid)
            for data in chatbot.ask(msg, conversation_id = all_convo[gid]):
